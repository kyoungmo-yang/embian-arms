...
function (angular, app, _, kbn, moment) {
  ...
  var module = angular.module('kibana.panels.stable', []);
  app.useModule(module);

  module.controller('stable', function($rootScope, $scope, $modal, $q, $compile, $timeout,
    ...
    $scope.panelMeta = {
      ...
      editorTabs : [
        {
          title:'Paging',
          src: 'app/panels/stable/pagination.html'
        },
        {
          title:'Queries',
    ...
    $scope.init = function () {
      $scope.pagination = 1;
      $scope.pagesize = ['30', '50', '100', '200'];
      $scope.pagesize_select = $scope.pagesize[0];
      ...
      $scope.$on('__search',function(){
    	  $scope.first_page();
		  querySrv.resolve().then(function(){$scope.get_data();});
	  });

      $scope.fields = fields;
    };

    $scope.origin = function(sources) {
    	var logtype;
		var hostname;
		var servername;
    	...
    	for(var k in sources) {
    		...
    		else if(k == 'payload.hostname') {
    			hostname = sources[k];
    		}
    		else if(k == 'payload.service') {
    			servername = sources[k];
    		}
    	}
    	return servername + ' > ' + hostname + ' > ' + logtype;
    };
    
    $scope.origin_msg = function(sources) {
    	var msg;
    	
    	for(var k in sources) {
    		if(k == 'payload.message') {
    			msg = sources[k];
    		}
    	}
    	return msg;
    ...
    $scope.pre_page = function(offset, pagination) {
    	$scope.pagination = pagination - 1;
    	$scope.panel.from = $scope.panel.from - $scope.panel.size;
    	querySrv.resolve().then(function(){$scope.get_data();});
    };
    
    $scope.next_page = function(offset, pagination) {
    	$scope.pagination = pagination + 1;
    	$scope.panel.from = $scope.panel.from + $scope.data.length;
    	querySrv.resolve().then(function(){$scope.get_data();});
    };
    
    $scope.change_pagesize = function () {
    	$scope.panel.size = parseInt($scope.pagesize_select);
    	querySrv.resolve().then(function(){$scope.get_data();});
    	$scope.pagination = 1;
	};
...
//        .from($scope.from_val)
      ...
      request.doSearch().then(function(results) {
        ...
        if (($scope.data.length < $scope.panel.size*$scope.panel.pages ||
		  ...
		  !((_.contains("timestamp",$scope.panel.sort[0])) && $scope.panel.sort[1] === 'desc')) &&
  ...
  module.directive('myRepeatDirective', function() {
	  return function(scope, element, attrs) {
		    if (scope.$last){
		    	  setTimeout(function() {
		    		  var keywords = $("#keywords-tmp").val().split(' ');
		    		  for(var i=0; i<keywords.length ; i++) {
					  		$(".highlight-check").highlight(keywords[i]);
					  }
		    	  }, 0);
		    }
	  };
