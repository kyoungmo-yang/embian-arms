...
function (angular, $, kbn, _, config, moment, Modernizr) {
  ...
  module.service('dashboard', function(
    ...
    $rootScope.$on('$routeChangeSuccess',function() {
      ...
      self.current = {};
      // self.indices = [];
	  self.indices = config.elasticsearch_indices;
      route();
    });
    ...
    this.dash_load = function(dashboard) {
      ...
      var refreshfilter = self._loadRefreshFilter();
      if(refreshfilter) {
    	  if(refreshfilter == 'false') {
    		  self.set_interval(false);
    	  } else if(/[0-9]+(s|m|h)/.test(refreshfilter)) {
    		  self.set_interval(refreshfilter, false);
    	  }
           
      } else if(dashboard.refresh) {
    ...
    this.set_interval = function (interval, doSave) {
    	doSave = typeof doSave !== 'undefined' ? doSave : true;
    	if(doSave) {
      	  this._saveRefreshFilter(interval);
        }
	...
	this._saveRefreshFilter = function(refresh) {
	  	this.setCookie('__srm_refresh', refresh, 365);
	};
	
	
	this._loadRefreshFilter = function() {
			var filter = this.getCookie('__srm_refresh');
			if(!filter) {
				return false;
			}
